...
use alloc::vec;
use fuser::{Filesystem, ReplyAttr, ReplyData, ReplyEntry, Request, ReplyWrite, ReplyDirectory, ReplyCreate};
use libc::{ENOENT};
use std::ffi::OsStr;
...
use crate::fuse::inode::{dbfs_fuse_create, dbfs_fuse_getattr, dbfs_fuse_lookup};
...
impl Filesystem for DbfsFuse {
    ...
    fn lookup(&mut self, _req: &Request, parent: u64, name: &OsStr, reply: ReplyEntry) {
        let res = dbfs_fuse_lookup(parent-1, name.to_str().unwrap());
        match res {
    ...
    fn getattr(&mut self, _req: &Request<'_>, ino: u64, reply: ReplyAttr) {
        let res = dbfs_fuse_getattr(ino-1);
        match res {
    ...
    fn read(&mut self, _req: &Request<'_>, ino: u64, _fh: u64, offset: i64, size: u32, _flags: i32, _lock_owner: Option<u64>, reply: ReplyData) {
        ...
        let res = dbfs_fuse_read(ino-1, offset, size,data.as_mut_slice());
        match res {
    ...
    fn write(&mut self, _req: &Request<'_>, ino: u64, _fh: u64, offset: i64, data: &[u8], _write_flags: u32, _flags: i32, _lock_owner: Option<u64>, reply: ReplyWrite) {
        let res = dbfs_fuse_write(ino-1, offset, data);
        match res {
    ...
    fn readdir(&mut self, _req: &Request<'_>, ino: u64, _fh: u64, offset: i64, reply: ReplyDirectory) {
        dbfs_fuse_readdir(ino-1,offset,reply)
    }

    fn create(&mut self, req: &Request<'_>, parent: u64, name: &OsStr, mode: u32, _umask: u32, flags: i32, reply: ReplyCreate) {
        let res = dbfs_fuse_create(req,parent-1,name.to_str().unwrap(),mode,flags);
        match res {
