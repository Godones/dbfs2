...
use rvfs::{create_tmp_inode_from_sb_blk, DirEntry, FileMode, Inode, InodeMode, InodeOps, StrResult};
use spin::Mutex;
...
pub const DBFS_FILE_INODE_OPS: InodeOps = InodeOps::empty();
pub const DBFS_SYMLINK_INODE_OPS: InodeOps = InodeOps::empty();
...
fn dbfs_link(
    ...
    bucket.put(new_dentry.d_name.clone(),old_inode.number.to_be_bytes()).unwrap();
    tx.commit().unwrap();
...
fn dbfs_symlink(
    ...
    dbfs_common_create(dir, dentry, FileMode::FMODE_READ, InodeMode::S_SYMLINK, Some(target))
}
...
fn dbfs_common_create(
    ...
    parent.put(dentry.d_name.clone(),new_number.to_string()).unwrap();
    let new_inode = tx.create_bucket(new_number.to_string()).unwrap();
    new_inode.put("mode", inode_mode_from_file_mode(mode)).unwrap();
    new_inode.put("type", inode_type_from_inode_mode(inode_mode)).unwrap();
    ...
    inode.inode_ops = DBFS_FILE_INODE_OPS;
