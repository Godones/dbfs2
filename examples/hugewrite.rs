...
use dbfs2::fuse::sblk::dbfs_fuse_destroy;
...
fn main() {
    ...
    #[cfg(feature = "read")]
    {
        ...
        for i in 0..FILE_SIZE / BK {
            ...
            if i%512==0{
                println!("read 4GB cost:{:#?}", SystemTime::now().duration_since(start).unwrap());
                println!("Throughput: {} MB/s", ((i+1) * BK) as f64 / 1024.0 / 1024.0 / SystemTime::now().duration_since(start).unwrap().as_secs_f64());
            }
        }
        let end = SystemTime::now();
