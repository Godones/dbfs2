...
use crate::common::{DbfsAttr, DbfsError, DbfsResult, XattrNamespace, ACCESS_R_OK, ACCESS_W_OK, DbfsTimeSpec, DbfsPermission};
use crate::inode::{checkout_access, dbfs_common_attr};


pub fn dbfs_common_setattr() -> DbfsResult<DbfsAttr> {
    Err(DbfsError::Other)
}
...
pub fn dbfs_common_listxattr(_r_uid:u32, _r_gid:u32, ino:usize, buf: &mut [u8]) ->DbfsResult<usize>{
    let db = clone_db();
    let tx = db.tx(false).unwrap();
    let bucket = tx.get_bucket(ino.to_be_bytes())?;
    ...
    bucket.kv_pairs().for_each(|x|{
        if x.key().starts_with(b"user.") || x.key().starts_with(b"system.") || x.key().starts_with(b"trusted.") || x.key().starts_with(b"security.") {
            let tmp = size;
            size += x.key().len() + 1;
            if buf.len() >= size{
...
pub fn dbfs_common_removexattr(r_uid:u32,r_gid:u32,ino:usize,key:&str,ctime:usize)->DbfsResult<()>{
    let db = clone_db();
    let tx = db.tx(true).unwrap();
    let bucket = tx.get_bucket(ino.to_be_bytes())?;
    // checkout access
...
pub fn dbfs_common_chmod(r_uid:u32,r_gid:u32,ino:usize,mode:u16,ctime:usize)->DbfsResult<DbfsAttr>{
...
pub fn dbfs_common_chown(r_uid:u32, r_gid:u32, ino:usize, uid:Option<u32>, gid:Option<u32>, c_time:usize) ->DbfsResult<DbfsAttr>{
    let mut attr = dbfs_common_attr(ino).map_err(|_|DbfsError::Other)?;
    if let Some(gid) = gid{
    ...
    if perm.contains(DbfsPermission::S_IXUSR) || perm.contains(DbfsPermission::S_IXGRP) || perm.contains(DbfsPermission::S_IXOTH){
...
pub fn dbfs_common_utimens(r_uid:u32, r_gid:u32, ino:usize, atime:Option<usize>, mtime:Option<usize>, c_time:usize) ->DbfsResult<DbfsAttr>{
    let mut attr = dbfs_common_attr(ino).map_err(|_|DbfsError::Other)?;
    // checkout access
    let _uid = attr.uid;
    let _gid = attr.gid;
    ...
    if attr.uid != r_uid && ! checkout_access(
        attr.uid,
        attr.gid,
        mode&0o777,
        r_uid,
